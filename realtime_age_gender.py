    ...
    try:
        while True:
            ...
            if frame_index % frame_interval != 0:
                if preview:
                    cv2.imshow("Age/Gender realtime", frame)
            ...
            if preview:
                display_frame = draw_faces(frame=frame.copy(), faces=faces)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        default="age_gender_results.json",
        help="JSON output path. Default: age_gender_results.json",
    )
    parser.add_argument(
        "--detector-backend",
        default="opencv",
